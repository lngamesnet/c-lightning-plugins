...
# summary2->summary.py plugin extended to show detailed info about listed channels.https://github.com/lightningd/plugins/tree/master/summary
...
def summary2(plugin):
    ...
    for p in peers['peers']:
        ...
        for c in p['channels']:
            ...
            chans.append((c['total_msat'], to_us, to_them, p['id'],c['short_channel_id'], c['private'], p['connected']))
    ...
    if chans != []:
        ...
        for c in chans:
            ...
            our_len = int(round(int(c[1]) / biggest * 13))
            their_len = int(round(int(c[2]) / biggest * 13))
            divided = False
            ...
            if our_len == 0:
                ...
                left = "{:>13}".format(draw.left + draw.bar * (our_len - 1))
            ...
            if their_len == 0:
                ...
                right = "{:13}".format(draw.bar * (their_len - 1) + draw.right)
            ...
            totalsats=int(round(int(c[0]/1000)))
            outsats=int(round(int(c[1]/1000)))
            insats=int(round(int(c[2]/1000)))

            perout=int((outsats/totalsats)*100)
            perin=int((insats/totalsats)*100)

            s += ':total:'+format(totalsats)
            s += ':out:'+format(outsats)+'['+format(perout)+'%]'
            s += ':in:'+format(insats)+'['+format(perin)+'%]'
...
def init(options, configuration, plugin):
    plugin.currency = options['summary2-currency']
    plugin.currency_prefix = options['summary2-currency-prefix']
    info = plugin.rpc.getinfo()
    ...
    plugin.log("Plugin summary2.py initialized")
